...
public class EnumMaskPropertyDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (foldoutOpen)
        {
            ...
            {
                theEnum = DoNotOperator(Convert.ChangeType(1, enumUnderlyingType), enumUnderlyingType);
            }
            ...
            {
                theEnum = Convert.ChangeType(1, enumUnderlyingType);
            }
            ...
            if (layout == EnumMaskLayout.Vertical)
            {
                bool doesHaveAnyEffect = false;

                for (int j = 1; j < Enum.GetNames(fieldInfo.FieldType).Length; j++)
                {
                    if (IsSet(j))
                    {
                        ToggleIndex(0, false);
                        doesHaveAnyEffect = true;
                        break;
                    }
                }          

                if (!doesHaveAnyEffect)
                {
                    theEnum = Convert.ChangeType(1, enumUnderlyingType);
                }

                //Draw the list vertically
                for (int i = 1; i < Enum.GetNames(fieldInfo.FieldType).Length; i++)
                {
                    if (EditorGUI.Toggle(new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight * (1 + i), position.width, EditorGUIUtility.singleLineHeight), Enum.GetNames(fieldInfo.FieldType)[i], IsSet(i)))
                    {
                        ToggleIndex(i, true);
                    }
                    else
